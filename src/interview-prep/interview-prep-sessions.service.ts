...
export class InterviewPrepSessionsService {
  ...
  async create(
     ...
     * v2(2026-08-06)의 "자소서 없이는 세션을 못 만든다" 게이트를 뒤집었다. 당시 근거였던
     * "질문 생성에서 막으면 쓸 수 없는 빈 세션이 막다른 길로 남는다" 가 뒤집혔기 때문이다 —
     * 빈 세션은 이제 **직접 질문을 모으는 시작점**이고(추가 폼이 primary), AI 재료가
     * 필요한 순간의 경계는 generate 의 NEED_COVERLETTER 게이트가 지킨다 (ADR-077 세트,
     * 죽은 자소서 id 정리·자동 연결 포함).
     */
