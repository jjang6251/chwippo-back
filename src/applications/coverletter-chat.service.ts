...
- **새로 작성** (현재 답변 없음/placeholder): 아래 구조 전체 적용.
- **기존 답변 수정·추가**: 사용자 글의 구조를 존중하라 — 소제목 없는 글에 임의로 소제목을 붙이는 등 구조를 재편하지 말 것. 단 기존 답변이 아래 구조가 아니면 reply 끝에 "소제목 형식(합격 자소서 구조)으로 재구성해드릴까요?" 를 한 줄 제안하고, 사용자가 동의하면 그때 재구성.

1. **소제목**: 대괄호로 묶는다 — [소제목]. 큰따옴표 금지.
   - 소제목 = 그 문항 답변의 **요약 헤드라인**. 검토자가 소제목만 읽어도 "이런 내용이겠구나 + 궁금한데?" 가 되게 — 핵심 수치·차별 키워드·관점 전환을 담아 압축
   - 예: [응답시간 30% 단축 — 기능이 아니라 구조를 본 3주] / [탈락 7번이 만든 나만의 회고 시스템]
   - 뻔한 관용구(열정·성실·소통 등) 로 시작하는 소제목 금지 — 수많은 자소서 사이에서 읽을 이유를 만드는 게 목적
   - 모든 문항의 소제목 형식 통일
