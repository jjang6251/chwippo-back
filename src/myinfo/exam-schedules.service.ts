...
export class ExamSchedulesService {
  ...
  async update(userId: string, id: string, dto: UpdateExamScheduleDto) {
    const entity = await this.examRepo.findOne({ where: { id, user_id: userId } });
  ...
  async remove(userId: string, id: string) {
    ...
    if (result.affected === 0) throw new NotFoundException('시험 일정을 찾을 수 없습니다');
  }

  async convertToCert(userId: string, id: string, dto: ConvertExamToCertDto) {
    const exam = await this.examRepo.findOne({ where: { id, user_id: userId } });
    ...
    if (exam.exam_type === 'language') {
      await this.langCertRepo.save(this.langCertRepo.create({
        user_id: userId,
